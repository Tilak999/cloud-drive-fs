...
export default class GdriveFS {
	...
	public async list(folderId?: string, query: string = "", pageToken: string = ''): Promise<{ files: File[]; nextPageToken?: string; incompleteSearch?: boolean }> {
		if (folderId === "root" || folderId == null) folderId = await this.setupRootFolder();
